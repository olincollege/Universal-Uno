 ...
 *
 * If a card is not allocated on the heap is attempted to be freed, the result
 * is undefined. Attempting to free a non-card, attempting to free a card that
 * has already been freed, and accessing a card after freeing will all result in
 * undefined behavior.
 *
 ...
 *
 * Given a deck and a card color and value, add that card to the end of the
 * deck. This function modified the deck in place (i.e., it does not create a
 * new copy of the deck to append to).
 *
 ...
 *
 * Create a new UNO deck that is has 108 cards with the standard card values in
 * an UNO deck including correct color and value. The memory for the UNO deck is
 ...
 *
 * Given a card, the deck the card was in, and the deck to move the card to,
 * move the card from one deck to the end of another. This function modified
 * each deck in place and does not create a new card. The function fixes the
 * linked list connections of the original deck and decreases the size by one,
 * and increases the size of the second deck by 1 after adding the card to the
 * end.
 *
 * The function has undefined behavior if the card, old_deck or new_deck does
 * not exist. The function also has undefined behavior if the card is not found
 * in the old_deck.
 *
 ...
 *
 * Given a deck and a index that is less than the size of the deck return a
 * pointer to the card at the specific index. If the index is outboards of the
 * deck, the function returns NULL.
 *
 ...
 * end of list. This function modified the deck in place without changing the
 * value of the card or amount of the cards.
 *
 ...
 * only the card values (not the memory locations of the deck/cards) are
 ...
 *
 * Given a pointer to a deck, find and return a pointer to a card that matches
 * the inputted color and value
 *
 * @param deck__ A pointer to the deck to append to.
 * @param color The color for the new card.
 * @param value The value for the new card.
 ...
 * head of the old deck to the final value of the new deck, then set the old
 * deck head to NULL and the size to 0. The function does not create any new
 * cards or decks, only moves one deck to the end of another.
 *
 ...
 *
 * Given a pointer to the game state, check the state of the draw pile. If the
 * draw pile is greater than 1 card, return 0 to indicate it does not needs to
 * be refilled. If the draw pile is 1 or below, return 1 to indicate the draw
 * pile needs to be refilled.
 *
 * @param state A pointer to the current game state.
 * @return 1 if the draw pile needs to refilled or 0 if the draw pile does not
 * need to be refilled.
 */
 ...
 *
 * Given a pointer to the game state, refill the draw pile by shuffling the
 * discard pile, and moving the cards to the end of the draw pile. The discard
 * pile should be empty after this function is run, and no cards or decks are
 * created.
 *
 * @param state A pointer to the current game state.
 */
 ...
 *
 * Given a pointer to the game state, move the top card from the draw pile to
 * the current play's hand.
 *
 * @param state A pointer to the current game state.
 */
 ...
 *
 * Given an number for the player, create a new player on the heap holding the
 * player's number, an empty deck for the player's hand, and the other player
 * values set to NULL or -1. The function returns a pointer to that player. The
 * caller of this function is considered the owner of the new player and is
 * responsible for cleaning it up when done with it.
 *
 ...
 * taken up by that player. The function should also free the player's hand
 * which was also allocated on the heap. Generally, this function should only be
 * called in functions that originally allocated the player with make_player, or
 * as part of another free_* function whose corresponding make_* function and/or
 * other functions allocate cards.
 *
 * If a player is not allocated on the heap is attempted to be freed, the result
 * is undefined. Attempting to free a non-player, attempting to free a player
 * that has already been freed, and accessing the player after freeing will all
 ...
 *
 * Given an number of players, create a new order on the heap and return a
 * pointer to that order. The order will create a circular doubly linked list of
 * players. The caller of this function is considered the owner of the new order
 * and is responsible for cleaning it up when done with it.
 *
 ...
 * taken up by that order. Any players contained in the deck will be freed, as
 * will the order itself. Undefined behavior results if the passed pointer does
 * not point to a order allocated on the heap or has already been freed.
 * Attempting to access the order after it has been freed also results in
 * undefined behavior.
 *
 ...
 *
 * Given a pointer to the state of the game check if the current player has zero
 * cards left and has won the game.
 *
 * @param state A pointer to the current game state.
